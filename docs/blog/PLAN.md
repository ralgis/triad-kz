...
| Primary search engines | **Google (~74%, primary), Yandex (~25%, secondary)** в KZ ([StatCounter 2025](https://gs.statcounter.com/search-engine-market-share/all/kazakhstan); Yandex trending DOWN outside RU) |
...
бетонного кольца КС10.9»). Этого мало — **значительная часть поискового
трафика приходится на информационные запросы**: «как выбрать ФБС блоки
для подвала», «расшифровка маркировки бетонных колец», «расчёт количества
колец для септика», «отличия ГОСТ 8020-90 и серии 3.900.1-14», «марки
бетона для фундамента». Generalist-исследование 2009 (всё ещё cited в
SEO-литературе): ~80% запросов informational, ~20% navigational+
transactional ([Search Engine Land](https://searchengineland.com/study-80-percent-of-searches-are-informational-20-are-navigational-or-transactional-13745)).
Точный split для B2B-ЖБИ KZ замерим через Search Console после
3-6 месяцев публикаций — пока полагаемся на общую оценку.

Блог ловит эти запросы и через внутреннюю перелинковку гонит трафик
в каталог.
...
3. **Topical authority** — Google (June 2025 Core Update) и Yandex
   склеивают домен с экспертной нишей, что бустит ранжирование и
   каталог-страниц

---

## 2. SEO-приоритеты (агентская агрегация, не статистика)

> **Disclaimer:** доли ниже — это **эвристическая агрегация** из разных
> SEO-источников 2024-2026 ([Backlinko ranking factors](https://backlinko.com/google-ranking-factors),
> [Linkbuilder.io 2025](https://linkbuilder.io/google-ranking-factors/),
> агентские блоги), не controlled-study. Google не раскрывает веса своего
> алгоритма публично. Цифры дают порядок величины для приоритезации
> ресурсов, не точную метрику для оптимизации.
>
> Известный факт: backlink вес упал с историч. ~50% до ~13%; topical
> authority вес вырос до ~13% после June 2025 Core Update (источник:
> [Linkbuilder.io 2025](https://linkbuilder.io/google-ranking-factors/)).

| Фактор | Эвристический вес | Что делаем |
|---|---|---|
| Качество и глубина контента | dominant (top-1 во всех опросах) | Длина по purpose, не по числу. Реальные ГОСТы, реальные размеры, реальные фото. |
| Off-page (backlinks, brand mentions) | high (~13% per Linkbuilder.io 2025) | Out of scope этого плана — PR + партнёры. |
| Topical authority | high (~13% post June 2025 Core Update) | Pillar+cluster архитектура, M2M товары/ГОСТы. |
| Тех. SEO (CWV, sitemap, structure, indexing) | medium | Phase 1 P0 + 2 P1 — фундамент. |
| Schema.org структурированные данные | low-medium | Article + Organization + Breadcrumb. Не самоцель. |
...
> **Wordstat-данные пока неизвестны.** Wordstat требует авторизованного
> доступа — цифры частотности заполнит контент-маркетолог через
> [wordstat.yandex.ru](https://wordstat.yandex.ru/) (фильтр регион
> «Казахстан» / «Алматы») перед контент-планированием. Указанные
> названия pillars — рабочие, корректируются после Wordstat-аудита.

| Категория | Рабочий pillar | Wordstat (TBD) |
|---|---|---|
| Бетонные кольца | «Бетонные кольца КС: полный справочник» | ? |
| ФБС блоки | «Фундаментные блоки ФБС: маркировка, ГОСТ, монтаж» | ? |
| Плиты перекрытия | «Плиты перекрытия колодцев: ПП, серия 3.900.1-14» | ? |
| Теплотрассы | «Лотки и опорные подушки для теплотрасс» | ? |
| ГОСТы и серии | «Справочник ЖБИ ГОСТов» | ? |
...
**Почему `\p{P}` а не `\b`:** `\b` в PCRE2 (PHP 7.3+) с флагом `/u`
**формально работает** для кириллицы ([PHP Manual: Unicode regexp](https://www.php.net/manual/en/regexp.reference.unicode.php)).
Выбор `\p{P}` (Unicode punctuation class) — стилистический: явнее
выражает намерение «разделять на whitespace + punctuation», устойчивее
к edge cases (составные/дефисированные слова, экзотические разделители)
и не зависит от UCD-таблицы PCRE-версии хостинга.
...
```json
{
  ...
  "about": [
    { "@type": "Product", "@id": "https://triad.kz/catalog/{category-slug}/{product-slug}#product" },
...
### 9.2. Что auto-rendering, что Filament validation

Чтобы не дублировать:

| Ссылка | Реализация |
|---|---|
| Cluster → pillar (внизу cluster-статьи) | **Auto-render** (`x-cluster-of-pillar`) — генерится из `pillar_id` FK |
| Pillar → все кластеры (внутри pillar-статьи) | **Auto-render** (`x-pillar-clusters`) — генерится из обратной выборки |
| In-text inline links на товары / ГОСТы | **Form validation** в Filament — admin вставляет ссылки руками через InternalLinkPicker, форма проверяет ≥2 product links на cluster, ≥1 product link на pillar |
| Anchor text inline ссылок | **Form warning** — generic «подробнее» / «здесь» вызывает warning |

То есть структурные ссылки авто-рендерятся (нет смысла валидировать —
они всегда есть), а inline-ссылки в теле валидируются.
...
Google использует CWV как ranking factor (Page Experience update);
Yandex с 2024 явно сигнализирует CWV как важный фактор. Цели:
...
| `article_read_25pct` / `_50pct` / `_75pct` / `_100pct` | Scroll depth (4 события) |
...
Эти events отражают engagement, который Yandex кэптурит как input
для поведенческих ranking factors (Crystal / Y1-семейство сигналов).
Прямой документированный мапинг «event → rank delta» отсутствует —
это implicit signal, не explicit input.
...
**Правильно:** «популярные» брать из [**Yandex Metrika Reports API**](https://yandex.com/dev/metrika/en/stat/)
(тж. Stat/Reporting API) или GA4 Data API раз в N часов в cache (Redis).
Y.Metрика уже:
- фильтрует ботов
...
Альтернативы Y.Metрика API в Metrika ecosystem: **Logs API**
(non-aggregated raw data) для глубокого breakdown — нам пока не нужен.
...
- **Author entity** (model + Filament + ProfilePage + `/blog/author` URL) — решено не делать. Publisher-only режим. Реальное влияние 1-3%, не окупает operational cost для не-YMYL ниши. (Model + Filament Resource были прототипированы и удалены до production-commit'а, ProfilePage не строилась вовсе.)
...
- **SpeakableSpecification schema** — НЕ deprecated, но BETA и ограничено scope'ом news content ([Google Speakable docs](https://developers.google.com/search/docs/appearance/structured-data/speakable)). Для B2B-ЖБИ блога — outside scope (news only), поэтому не добавляем. Возможно вернуть когда расширят scope.
...
### v2.1 (2026-05-30, later) — fact-check pass

После self-critique пересмотрел утверждения через web research. Что нашёл:

**Corrected (был неправ):**
- **KZ search market share** — был «Yandex ~60% / Google ~40%», на самом деле **Google ~74%, Yandex ~25%** ([StatCounter](https://gs.statcounter.com/search-engine-market-share/all/kazakhstan)). Yandex declining в KZ. Это означает что Google теперь primary, Yandex secondary — план обновлён.
- **Code bug в §6.2** — `$word_count` → `$this->word_count` (undefined variable в shown пример).
- **«~80% informational queries»** — есть source ([SEL 2009 study](https://searchengineland.com/study-80-percent-of-searches-are-informational-20-are-navigational-or-transactional-13745)), всё ещё cited. Reframed с источником.
- **`\b` regex Cyrillic «ненадёжен»** — overclaim. С `/u` в PCRE2 работает ([PHP Manual](https://www.php.net/manual/en/regexp.reference.unicode.php)). Выбор `\p{P}` — clarity choice, не fix.
- **«Yandex Metrika Reports API» — wrong name** — self-critique был неправ. yandex.com/dev/metrika явно использует «Reports API» как термин. Восстановил.
- **Calibrated percentages** — заменено на «agency aggregation» disclaimer с источниками (Linkbuilder.io 2025: backlinks 13%, topical authority 13% post June 2025 Core Update).
- **Wordstat numbers** — были выдуманные (12k/8k/3k/2k). Помечено TBD до Wordstat-аудита.
- **SpeakableSpecification** — было «Google убрал поддержку 2023», на самом деле НЕ deprecated, просто BETA + scope only news ([Google docs](https://developers.google.com/search/docs/appearance/structured-data/speakable)). Корректная формулировка: не подходит для нашего use case (B2B-блог не news).

**Mellowed (overstatement → calibrated):**
- «Yandex с 2024 жёстче Google карает CWV» → «Google + Yandex оба используют CWV как ranking factor»
- «Metрика использует events в re-ranking» → «events отражают engagement, который кэптурится как input для behavioral signals»
- «Author entity removed» в change log → «решено не делать» (ProfilePage никогда не строилась, инфраструктура была прототипирована и снесена pre-commit)

**Added в CLAUDE.md (отсутствовали):**
- §11 Content sanitization — security gap (mews/purifier установлен, но не вызывается)
- §12 Preview черновика — стратегия для admin preview
- §13 OG image fallback chain — 4-уровневый fallback
- §14 SQLite vs MySQL/MariaDB нюансы (JSON, FULLTEXT, indexes)

Подробности в [CLAUDE.md §16 change log](./CLAUDE.md#16-change-log).
